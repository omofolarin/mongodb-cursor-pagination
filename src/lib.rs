...
impl PaginatedCursor {
    ...
    pub(crate) fn create_from_doc(&self, doc: &Document) -> String {
        ...
        if let Some(sort) = &self.options.sort {
            ...
            let mut buf = Vec::new();
            Document::to_writer(&only_sort_keys, &mut buf).unwrap();
...
pub async fn count_documents<T>(
    ...
    let mut count_options = cursor.options.clone();
    ...
    T: Serialize + DeserializeOwned + Unpin + Send + Sync,
{
    ...
    let total_count: u64 = count_documents(cursor, collection, filter).await?;
    ...
    if total_count > 0 {
        // build the cursor
        let query_doc = cursor.get_query(filter)?;
        let mut options = cursor.options.clone();
        let skip_value: u64 = if let Some(s) = options.skip { s } else { 0 };
        if cursor.has_cursor || skip_value == 0 {
            options.skip = None;
        } else {
            has_skip = true;
        }
        // let has_previous
        let is_previous_query = cursor.has_cursor && cursor.direction == CursorDirections::Previous;
        ...
        let mut mongodb_cursor = collection
            .find(query_doc, options)
            .await
            .map_err(|e| CursorError::InvalidCursor(e.to_string()))?;

        while let Some(result) = mongodb_cursor.next().await {
            match result {
                Ok(doc) => {
                    let item = bson::to_document(&doc).unwrap();
                    edges.push(Edge {
                        cursor: cursor.create_from_doc(&item),
                    });
                    items.push(doc);
                }
                Err(error) => {
        ...
        if has_skip {
            ...
            has_more = items.len() > (cursor.options.limit.unwrap() - 1) as usize;
            has_previous_page = (cursor.has_cursor && cursor.direction == CursorDirections::Next)
                || (is_previous_query && has_more);
            has_next_page = (cursor.direction == CursorDirections::Next && has_more)
                || (is_previous_query && cursor.has_cursor);
        }
...
pub async fn estimated_document_count<T>(
    ...
    let count_options = cursor.options.clone();
    let total_count: u64 = collection
...
fn map_from_base64(base64_string: String) -> Result<Document, CursorError> {
    ...
    // decode from bson
    let cursor_doc = bson::from_reader(&mut Cursor::new(&decoded)).unwrap();
    Ok(cursor_doc)
}
